...
const ExpensesDetail = ({ expense }: ExpenseDetailsProps) => {
  ...
  const trailingActions = () => {
    return (
      <TrailingActions>
        <SwipeAction
          onClick={() => {
            dispatch({ type: "remove-expanse", payload: { id: expense.id } });
          }}
          destructive={true}
        >
          Eliminar
  ...
  return (
    <SwipeableList>
      ...
      >
        <div className="bg-white shadow-lg p-10 w-full border-b border-gray-200 flex gap-5 items-center cursor-grab">
          <div>
            <img
